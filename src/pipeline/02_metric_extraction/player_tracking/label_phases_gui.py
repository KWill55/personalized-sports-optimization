...
class VideoAnnotator(QWidget):
    def __init__(self):
        ...
        self.current_marks = {"windup_start": None, "release_frame": None, "followthrough_end": None}
        self.mark_mode = None
    ...
    def open_video(self, path: Path):
        ...
        self.current_frame = 0
        self.current_marks = self.phase_data.get(path.name, {"windup_start": None, "release_frame": None, "followthrough_end": None})
    ...
    def step_frame(self, offset: int):
        ...
        if offset < 0:
            # Go back two because read() will advance one
            new_frame = max(0, curr + offset - 1)
        elif offset > 0:
            # Go forward zero because read() already advances one
