...
# Support direct execution: python3 src/primary_measurements/pose_3d_node.py
SRC_ROOT = Path(__file__).resolve().parents[1]
if str(SRC_ROOT) not in sys.path:
    sys.path.append(str(SRC_ROOT))

from primary_measurements.pose_3d_lib import run_pose_3d_pipeline
from secondary_measurements.kinematics_3d_lib import run_kinematics_3d_pipeline
from utils.io_utils import load_config


class Pose3dNode:
    """Orchestrates 3D pose reconstruction from stereo 2D keypoints."""
    ...
    def run(self) -> dict[str, Any]:
        return run_pose_3d_pipeline(self.cfg, do_triangulation=True, do_kinematics=False)

    def triangulate(self) -> dict[str, Any]:
        return run_pose_3d_pipeline(self.cfg, do_triangulation=True, do_kinematics=False)

    def compute_secondary(self) -> dict[str, Any]:
