...
Usage:
    - place vertical line object in camera view to ground both cameras 
...
def _find_project_config() -> Path:
    ...
    for parent in [here, *here.parents]:
        candidate = parent / "project_config.yaml"
        if candidate.exists():
            return candidate
...
# Camera indices
LEFT_CAM_INDEX = int(_cfg_get(project_cfg, ["left_cam_index"], 0))
RIGHT_CAM_INDEX = int(_cfg_get(project_cfg, ["right_cam_index"], 1))
BALL_CAM_INDEX = int(_cfg_get(project_cfg, ["ball_cam_index", "third_cam_index"], 2))

# Calibration parameters
CHECKERBOARD = tuple(_cfg_get(project_cfg, ["inner_corners"], [10, 7]))  # (columns, rows)
SQUARE_SIZE = float(_cfg_get(project_cfg, ["square_size_cm", "square_size_in"], 1.0))
WINDOW_SIZE = int(_cfg_get(project_cfg, ["success_window"], 50))

# Video parameters
stereo_res = _cfg_get(project_cfg, ["uncropped_stereo_resolution"], [1280, 720])
CAM_RESOLUTION = (int(stereo_res[0]), int(stereo_res[1]))
CROP_SIZE = tuple(_cfg_get(project_cfg, ["cropped_stereo_resolution", "crop_size"], [0, 0]))
PLAYER_TRACKING_FPS = int(_cfg_get(project_cfg, ["player_tracking_fps"], 30))

# Detection threshold
THRESHOLD_DETECT = float(_cfg_get(project_cfg, ["threshold_detect"], 0.7))
