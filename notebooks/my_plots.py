...
# ???
# -------------------------------------------------------------------
def plot_angles_over_frames(angles_long, session_id: str, use_seconds: bool = False, files=None):
    """
    Interactive: shows ALL angles for ONE throw at a time.
    Use the slider to step through throws (by 'file').
    """
    ...
    fig = px.line(
        df,
        x=xcol, # what goes on the x-axis
        y="value", # what goes on the y-axis
        color="angle", # how traces are grouped/colored
        animation_frame="file", # which variable controls the slider/animation
        animation_group="angle", # keeps same color/trace identity across frames
        hover_data=["file","angle","frame","session"] # what extra info appears when hovering
    )
    fig.update_layout(
    ...
    # Optional: add reference line
    fig.add_vline(
        x=50,
        line_color="red",
        line_dash="dash",
        annotation_text="Release Frame (dummy)",
